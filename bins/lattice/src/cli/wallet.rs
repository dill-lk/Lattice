...
pub fn import_from_private_key(private_key: &str, output: &str) -> Result<()> {
    ...
    let _secret_bytes = hex::decode(key_hex).map_err(|_| anyhow!("Invalid hex encoding"))?;
...
pub fn list_wallets() -> Result<()> {
    println!();
    println!("  {}", "Lattice Keystores in Current Directory".bold().cyan());
            ...
            if path.extension().map_or(false, |ext| ext == "json") {
                if let Ok(keystore) = Keystore::load_from_file(&path) {
                    println!("  • {} -> {}", 
                        ...
                        keystore.address().to_string().cyan()
                    );
                    found += 1;
                }
            }
    ...
    if found == 0 {
        println!("  No valid keystores (*.json) found in the current directory.");
    }
    println!();
    Ok(())
}
...
pub fn delete_wallet(wallet_path: &str) -> Result<()> {
    ...
    println!();
    println!("  {}", "Delete Keystore File".bold().red());
    println!("  {}", "─".repeat(50).dimmed());
    println!();
    println!("  Keystore File: {}", wallet_path.white());
    println!("  {} This will permanently delete the private key file!", "WARNING:".red().bold());
    println!();
    ...
    if confirmed {
        ...
        println!("  {} Keystore file deleted.", "✓".green());
    } else {
